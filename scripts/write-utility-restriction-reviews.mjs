...
const reviewPath = process.env.UTILITY_REVIEW_OUTPUT_PATH || path.join(dataDir, "utility_restriction_reviews.json");
...
const concurrency = Math.max(1, Number(process.env.UTILITY_REVIEW_WRITE_CONCURRENCY || 4));
const maxAttempts = Math.max(1, Number(process.env.UTILITY_REVIEW_WRITE_ATTEMPTS || 8));
...
const rows = reviews.filter((row) => row?.opportunityId && row?.utilityRestrictionReview);
const statusCounts = countBy(rows, (row) => row.utilityRestrictionReview.restrictionStatus || "unknown");

await mapWithConcurrency(rows, concurrency, writeReviewRow);

console.log("Utility restriction reviews written to DynamoDB.");
    ...
    await db.send(
      new UpdateCommand({
        ...
        ExpressionAttributeValues: {
          ":review": row.utilityRestrictionReview,
          ":updatedAt": reviewedAt,
          ":schemaVersion": artifact.schemaVersion || "utility-restriction-review-v1"
        }
      })
    );
  });
...
function printHelp() {
  console.log(`Usage: npm run matching:utility-reviews:write

Write an existing utility restriction review artifact to DynamoDB.

Environment:
  UTILITY_REVIEW_OUTPUT_PATH       Review JSON path. Default: data/utility_restriction_reviews.json.
  UTILITY_REVIEW_WRITE_CONCURRENCY DynamoDB update concurrency. Default: 4.
  UTILITY_REVIEW_WRITE_ATTEMPTS    Retry attempts per row. Default: 8.
