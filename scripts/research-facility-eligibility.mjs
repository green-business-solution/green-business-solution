...
import { inferFacilityRequirements } from "../apps/api/server/matching/facilityEligibility.mjs";
...
const fetchAttempts = Math.max(1, Number(process.env.FACILITY_REVIEW_FETCH_ATTEMPTS || 3));
const fetchRetryDelayMs = Math.max(0, Number(process.env.FACILITY_REVIEW_FETCH_RETRY_DELAY_MS || 30000));
...
async function fetchSourceText(url) {
  return fetchSourceTextWithRetry(url, {
    ...
    userAgent: "RetroFi facility eligibility review/1.0"
  });
}
...
Environment:
  ...
  FACILITY_REVIEW_FETCH_ATTEMPTS=3    Attempts per source URL. HTTP 429/5xx and timeouts are retried.
  FACILITY_REVIEW_FETCH_RETRY_DELAY_MS=30000 Base delay before retrying rate-limited or transient fetches.
