...
- Triple-verified the copied DynamoDB tables, current S3 objects, and versioned S3 histories before destructive cleanup.
- Deleted the remaining old-account RetroFi/GBS CloudFormation stacks, copied data stores, old S3 buckets, old ACM certificate, and old hosted zone after live production was confirmed on the new account.
...
- Added the missing CloudFormation template-summary permission required by GitHub Actions deploys.
...
- Imported the GPT Pro repair results into a tracked artifact with 19 promoted tax-rule candidates, one suppression, two input-gated records, one routed non-retrofit tax workflow, and 144 source documents.
- Added a tax gap repair intake report summarizing promoted rule candidates by model kind and runtime gate while keeping all repaired tax records excluded from customer-facing totals by default.
...
- Imported 58 grant package repairs, 15 deep probability repairs, and 51 realistic grant test-profile repairs into durable data artifacts.
- Applied the repairs to v2 incentive packages and regenerated sample grant/tax coverage plus public test-case savings fixtures.
...
- Trimmed the before-click picker vertical spacing further so the `Show more retrofits` control stays visible in a non-fullscreen desktop window.
...
- Shortened the customer Process onboarding Step 4 copy to `Receive implementation and application support`.
- Updated the customer Process onboarding Step 4 copy to say users receive automated retrofit implementation and rebate application support.
...
- Implemented the read-only Profile tab in the user preview, showing existing account, intake, site, utility, recommendation, tax, synthetic dashboard, and preview-assumption data without exposing raw storage keys or source snippets.
