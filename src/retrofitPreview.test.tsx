...
describe("retrofit recommendations preview", () => {
  ...
  it("renders the selected retrofit estimate workspace with the new tab set and safety gates", async () => {
    ...
    const workspaceSource = source.slice(workspaceStart, source.indexOf("function OpportunityPreviewRow", workspaceStart));
    ...
    const rightRailStart = workspaceSource.indexOf("<aside className=\"estimate-right-rail\"");
    const rightRailSource = workspaceSource.slice(rightRailStart, workspaceSource.indexOf("</aside>", rightRailStart));
    ...
    expect(rightRailSource).toContain("Confirm & move to next step");
    expect(rightRailSource).toContain("Discard changes");
    expect(rightRailSource).toContain("Estimate step in progress");
    expect(rightRailSource).toContain("Auto-saved just now");
    expect(rightRailSource).not.toContain("Selected opportunities");
    expect(rightRailSource).not.toContain("Current scenario");
    expect(rightRailSource).not.toContain("Download estimate");
    expect(rightRailSource).not.toContain("PDF summary");
    expect(rightRailSource).not.toContain("Application overview");
    expect(rightRailSource).not.toContain("Impact overview");
    expect(rightRailSource).not.toContain("Financial");
