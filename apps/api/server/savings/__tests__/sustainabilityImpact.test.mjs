...
describe("sustainability impact calculations", () => {
  it("returns numeric direct inputs and operational CO2e from bill deltas", () => {
    ...
    const expectedKg =
      1000 * electricityFactor.kgPerKwh + 12 * gasFactor.kgCo2ePerTherm;
    ...
    expect(impact.metrics.waterConservationGallonsPerYear.provenanceState).toBe(
      "source_calculated",
    );
    expect(impact.metrics.scope1ThermReductionPerYear.value).toBe(12);
    expect(impact.metrics.scope1ThermReductionPerYear.provenanceState).toBe(
      "source_calculated",
    );
    expect(impact.metrics.scope2ElectricityReductionKwhPerYear.value).toBe(
      1000,
    );
    expect(
      impact.metrics.scope2ElectricityReductionKwhPerYear.provenanceState,
    ).toBe("source_calculated");
    expect(impact.metrics.gridPeakDemandReductionKw.value).toBe(8);
    expect(
      impact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.value,
    ).toBeCloseTo(0.4612, 4);
    expect(
      impact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.provenanceState,
    ).toBe("source_calculated");
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.value,
    ).toBeCloseTo(expectedKg, 6);
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.provenanceState,
    ).toBe("source_calculated");
    expect(impact.metrics.annualOperationalCO2eReductionKgPerYear.unit).toBe(
      "kg CO2e/year",
    );
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.value,
    ).toBeCloseTo(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace
        .valueKgCO2ePerYear ?? 0,
      6,
    );
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.boundary
        .included,
    ).toEqual(
    ...
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components,
    ).toHaveLength(2);
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components[0]
        .valueKgCO2ePerYear,
    ).toBeGreaterThan(0);
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components[1]
        .valueKgCO2ePerYear,
    ).toBeGreaterThan(0);
  });
  ...
  it("returns numeric zeros with not-applicable provenance when a retrofit cannot affect a stream", () => {
    ...
    expect(impact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.value).toBe(
      0,
    );
    expect(
      impact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.provenanceState,
    ...
    expect(impact.metrics.gridPeakDemandReductionKw.provenanceState).toBe(
      "not_applicable",
    );
    expect(impact.metrics.annualOperationalCO2eReductionKgPerYear.value).toBe(
      0,
    );
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.provenanceState,
  ...
  it("keeps mixed not-applicable and source-calculated site EUI source-calculated in both directions", () => {
    ...
    expect(
      electricOnlyImpact.metrics.scope2ElectricityReductionKwhPerYear
        .provenanceState,
    ).toBe("source_calculated");
    expect(
      electricOnlyImpact.metrics.scope1ThermReductionPerYear.provenanceState,
    ).toBe("not_applicable");
    expect(
      electricOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.value,
    ).toBeCloseTo(0.0600512, 6);
    expect(
      electricOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear
        .provenanceState,
    ).toBe("source_calculated");
    expect(
      electricOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear
    ...
    expect(
      gasOnlyImpact.metrics.scope1ThermReductionPerYear.provenanceState,
    ).toBe("source_calculated");
    expect(
      gasOnlyImpact.metrics.scope2ElectricityReductionKwhPerYear
        .provenanceState,
    ).toBe("not_applicable");
    expect(
      gasOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.value,
    ).toBeCloseTo(0.12, 6);
    expect(
      gasOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear
        .provenanceState,
    ).toBe("source_calculated");
    expect(
      gasOnlyImpact.metrics.siteEuiReductionKbtuPerSquareFootPerYear.status,
    ).toBe("source_calculated");
  });
  ...
  it("estimates peak demand and CO2e from explicit fixture assumptions", () => {
    ...
    expect(impact.metrics.gridPeakDemandReductionKw.value).toBeCloseTo(
      4000 / (8760 * 0.8),
      6,
    );
    expect(impact.metrics.gridPeakDemandReductionKw.provenanceState).toBe(
    ...
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.value,
    ).toBeCloseTo(4000 * electricityFactor.kgPerKwh, 6);
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components[1]
        .factor.sourceType,
    ).toBe("regional_subregion");
    expect(
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components[1]
        .factor.fallbackUsed,
    ).toBe(false);
  ...
  it("preserves increased consumption signs and does not double count non-energy streams", () => {
    ...
    const componentTotal =
      impact.metrics.annualOperationalCO2eReductionKgPerYear.trace.components.reduce(
  ...
  it("keeps peak demand unavailable when a peak delta row has no numeric values", () => {
    ...
    expect(impact.metrics.gridPeakDemandReductionKw.value).toBe(0);
    expect(
      impact.metrics.gridPeakDemandReductionKw.trace.sourceDeltas,
    ).toHaveLength(1);
    expect(
      impact.metrics.gridPeakDemandReductionKw.trace.sourceDeltas[0].deltaValue,
    ).toBe("n/a");
  });
  ...
  it("attaches the sustainability impact contract to the admin savings preview", () => {
    ...
    expect(
      preview.sustainabilityImpact.metrics.scope2ElectricityReductionKwhPerYear
        .value,
    ).toBeGreaterThan(0);
    expect(
      preview.sustainabilityImpact.metrics
        .siteEuiReductionKbtuPerSquareFootPerYear.value,
    ).toBeGreaterThan(0);
    expect(
      preview.sustainabilityImpact.metrics
        .annualOperationalCO2eReductionKgPerYear.value,
    ).toBeGreaterThan(0);
    expect(
      preview.sustainabilityImpact.metrics.waterConservationGallonsPerYear
        .value,
    ).toBe(0);
    expect(
      preview.sustainabilityImpact.metrics.waterConservationGallonsPerYear
