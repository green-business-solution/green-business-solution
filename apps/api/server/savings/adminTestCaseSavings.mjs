...
import {
  buildSustainabilityImpact,
  normalizeSquareFootage,
} from "../sustainabilityImpact.mjs";
import { buildThreeYearFinancialValue } from "./threeYearFinancialValue.mjs";

export const ADMIN_TEST_CASE_SAVINGS_SCHEMA_VERSION =
  "admin-test-case-savings-v2";
...
const retrofitTemplates = {
  ...
  water_audit: waterTemplate({
    ...
    laborRequired: false,
    assumptions: [
      "Water audit preview models a conservative admin-entered annual water reduction.",
    ],
  }),
...
export function buildAdminTestCaseSavingsPreview({
  ...
  const selectedIncentiveRules = selectIncentiveRulesForRetrofitGroup(
    retrofitGroup,
    opportunityIncentiveRules,
  );
  const selectedIncentivePackages = selectV2PackagesForRetrofitGroup(
    retrofitGroup,
    opportunityIncentiveCalculationPackages,
  );
  ...
  const sustainabilityImpact = buildSustainabilityImpact({
    ...
    squareFootage: normalizeSquareFootage(
      normalizedProfile?.site?.squareFootage?.value,
    ),
    ...
    stateCode:
      normalizedProfile?.site?.geo?.stateCode ||
      normalizedProfile?.site?.addressStructured?.stateCode ||
      null,
  });
  ...
  return {
    ...
    upfrontCostCents: estimate.upfrontCostCents,
    upfrontSavingsCents:
      estimate.upfrontCostCents != null &&
      estimate.upfrontCostAfterSavingsCents != null
    ...
    monthlyRecurringSavingsCents:
      estimate.monthlyRecurringSavingsCents ?? estimate.monthlySavingsCents,
    annualRecurringSavingsCents:
      estimate.annualRecurringSavingsCents ?? estimate.annualSavingsCents,
    ...
    netMonthlyRecurringSavingsCents:
      estimate.netMonthlyRecurringSavingsCents ?? estimate.monthlySavingsCents,
    netAnnualRecurringSavingsCents:
      estimate.netAnnualRecurringSavingsCents ?? estimate.annualSavingsCents,
    ...
    paybackPeriodDetails: {
      ...
      normalizedAnnualRecurringSavingsCents:
        estimate.annualRecurringSavingsCents ??
        estimate.annualSavingsCents ??
        normalizeMonthlyRecurringSavings(
          estimate.monthlyRecurringSavingsCents ??
            estimate.monthlySavingsCents ??
            null,
        ),
    ...
    incentiveCalculationPackageCounts:
      estimate.incentiveCalculationPackageCounts || {
    ...
    unsupportedReason:
      estimate.status === "blocked"
        ? "Savings preview is blocked by missing fixture inputs."
        : null,
...
export function calculatePaybackPeriodYears({
  ...
  const annualRecurring =
    firstFiniteNumber(annualRecurringSavingsCents) ??
    normalizeMonthlyRecurringSavings(
      firstFiniteNumber(monthlyRecurringSavingsCents),
    );
...
function buildFixture({
  ...
  const selectedOpportunityIds = [
    ...new Set([
      ...
      ...opportunityIncentiveCalculationPackages.map(
        (pkg) => pkg.opportunity_id,
      ),
    ]),
  ];
  return {
    ...
    laborCostRules:
      template.laborRequired === false
        ? []
        : [buildLaborRule({ template, retrofitTypeId, geography })],
    ...
    taxOpportunitySpecificInputs:
      taxContext?.taxOpportunitySpecificInputs || [],
    ...
    grantOpportunitySpecificInputs:
      grantContext?.grantOpportunitySpecificInputs || [],
    opportunityIncentiveRules,
    opportunityIncentiveCalculationPackages,
    stackingRules: [],
  };
...
function selectIncentiveRulesForRetrofitGroup(
  retrofitGroup,
  opportunityIncentiveRules = [],
) {
  if (
    !retrofitGroup?.opportunities?.length ||
  ...
  return retrofitGroup.opportunities.flatMap(
    (opportunity) => rulesByOpportunityId.get(opportunity.opportunityId) || [],
  );
}
...
function buildLaborRule({ template, retrofitTypeId, geography }) {
  return {
    ...
    minimumContractorCostCents:
      template.laborRule?.minimumContractorCostCents || 0,
    countyLaborMultiplier: template.laborRule?.countyLaborMultiplier || 1,
    retrofitComplexityMultiplier:
      template.laborRule?.retrofitComplexityMultiplier || 1,
...
function savingsGeographyFromProfile(normalizedProfile) {
  ...
  return {
    ...
    postalCode:
      geo.zip5 || normalizedProfile?.site?.addressStructured?.zip5 || null,
...
function gasToElectricTemplate({
  thermsAvoided,
  addedKwh,
  equipmentCostCents,
  laborCostCents,
