...
export function evaluateOpportunityForUser(
  userMatchProfile,
  opportunity,
  matchProfile,
  { now = new Date() } = {},
) {
  const offerResults = matchProfile.offers.map((offer) =>
    evaluateOffer(userMatchProfile, opportunity, matchProfile, offer, { now }),
  );
...
function evaluateOffer(user, opportunity, profile, offer, { now }) {
  ...
  const hardFail = Object.values(checks).find(
    (check) => check.result === "fail",
  );
  const unknowns = Object.values(checks).filter(
  ...
  const eligibilityStatus = statusFor({
    hardFail,
    unknowns,
...
function awardAuditFields(opportunity) {
  return {
    ...
    requiresProgramApproval:
      typeof opportunity?.requiresProgramApproval === "boolean"
        ? opportunity.requiresProgramApproval
        : null,
...
function retrofitTypesFor(opportunity, profile) {
  return Array.isArray(profile.retrofitTypes)
    ? profile.retrofitTypes
    : classifyRetrofitsForOpportunity(opportunity, profile);
}

function evaluateAvailability(availability, now) {
  if (availability.normalizedStatus === "unavailable") {
    return fail(
      "Opportunity appears unavailable or the application deadline has passed.",
    );
  }
  if (availability.normalizedStatus === "upcoming") {
    return unknown(
      "Opportunity appears upcoming; application timing should be verified.",
    );
  }
  if (
    availability.normalizedStatus === "active" ||
    availability.normalizedStatus === "rolling"
  ) {
    return pass(
      availability.normalizedStatus === "rolling"
...
function evaluateGeography(user, geography) {
  ...
  if (!stateCode) {
    return unknown(
      "User site state is missing or was not parsed from the address.",
    );
  }
  const includedStates = geography.include
    .filter((item) => item.kind === "state")
    .map((item) => item.id);
  const isNationwide = geography.include.some(
    (item) => item.kind === "nationwide" || item.id === "US",
  );
  if (isNationwide || includedStates.includes(stateCode)) {
    return pass(
      isNationwide
        ? "Opportunity appears nationwide."
        : `Project site state ${stateCode} matches opportunity geography.`,
    );
  }
  return fail(
    `Project site state ${stateCode} does not match opportunity geography ${includedStates.join(", ")}.`,
  );
}
...
function evaluateUtility(user, utilityRequirements) {
  ...
  if (required.length === 0 && requiredNames.length === 0)
    return unknown(
      "Utility restriction is required but no utility could be normalized.",
    );
  ...
  if (required.includes(userUtilityId)) {
    return pass(
      `Self-reported utility matches ${UTILITY_DISPLAY_NAMES[userUtilityId] || userUtilityId}.`,
    );
  }
  const userUtilityName = UTILITY_DISPLAY_NAMES[userUtilityId] || userUtilityId;
  if (
    requiredNames.some((name) =>
      normalizedUtilityNameMatches(name, userUtilityName),
    )
  ) {
...
function normalizedUtilityNameMatches(requiredName, userUtilityName) {
  const required = String(requiredName || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
  const user = String(userUtilityName || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
  return Boolean(
    required && user && (required.includes(user) || user.includes(required)),
  );
}

function evaluateApplicant(user, applicant, offer) {
  const userTypes = user.business.organizationTypes || [];
  const eligibleTypes = unique([
    ...(offer.sectors || []),
    ...(applicant.eligibleOrganizationTypes || []),
  ]);
  if (
    applicant.residentialOnly &&
    !userTypes.includes("multifamily") &&
    !userTypes.includes("residential")
  ) {
    return fail(
      "Opportunity appears residential-only and the user profile is nonresidential.",
    );
  }
  ...
  if (userTypes.some((type) => eligibleTypes.includes(type))) {
    return pass(
      `Applicant type overlaps eligible sector: ${userTypes.filter((type) => eligibleTypes.includes(type)).join(", ")}.`,
    );
  }
  if (
    eligibleTypes.includes("commercial") &&
    !userTypes.includes("residential")
  ) {
    return pass(
  ...
  return fail(
    `Applicant type ${userTypes.join(", ") || "unknown"} does not match eligible sectors ${eligibleTypes.join(", ")}.`,
  );
}
...
function evaluateSite(user, site) {
  ...
  const facilityConfidence =
    typeof site.confidence === "number" ? site.confidence : 0;
  ...
  if (
    facilityEligibilityStatus === "broad_nonresidential" &&
    buildingTypesOverlap(
      userBuildingTypes,
      eligibleBuildingTypes,
      facilityEligibilityStatus,
    )
  ) {
    return pass(
  ...
  if (
    facilityEligibilityStatus === "broad_commercial" &&
    buildingTypesOverlap(
      userBuildingTypes,
      eligibleBuildingTypes,
      facilityEligibilityStatus,
  ...
  if (
    facilityEligibilityStatus === "broad_residential" &&
    buildingTypesOverlap(
      userBuildingTypes,
      eligibleBuildingTypes,
      facilityEligibilityStatus,
  ...
  if (
    ["broad_nonresidential", "broad_commercial", "broad_residential"].includes(
      facilityEligibilityStatus,
    )
  ) {
    ...
    if (facilityConfidence >= 0.7) {
      return fail(
        `User site or facility type (${userBuildingTypes.join(", ")}) does not match ${facilityEligibilityStatus} eligibility.`,
      );
    }
    return unknown(
      `Opportunity site or facility scope (${facilityEligibilityStatus}) does not directly match the user's site type.`,
    );
  }
  if (facilityEligibilityStatus !== "required") {
    return unknown(
      "Site or facility type restriction is still unknown after review.",
    );
  }
  if (eligibleBuildingTypes.length === 0) {
    return unknown(
      "Site or facility type is required but no specific eligible type was normalized.",
    );
  }
  if (
    buildingTypesOverlap(
    ...
    return pass(
      `Site or facility type matches: ${userBuildingTypes.filter((type) => eligibleBuildingTypes.includes(type)).join(", ") || userBuildingTypes.join(", ")}.`,
    );
  }
  if (userBuildingTypes.length === 0) {
    return unknown("User site or facility type is missing.");
  ...
  if (facilityConfidence >= 0.7) {
    return fail(
      `Opportunity site or facility specificity (${eligibleBuildingTypes.join(", ")}) does not match the user's site type (${userBuildingTypes.join(", ")}).`,
    );
  }
  return unknown(
    `Opportunity site or facility specificity (${eligibleBuildingTypes.join(", ")}) does not directly match the user's site type.`,
  );
}

function evaluateProject(user, project, offer) {
  const opportunityTechnologies = unique([
    ...(offer.technologies || []),
  ...
  return pass(
    `Opportunity technology is available for retrofit discovery: ${opportunityTechnologies.join(", ")}.`,
  );
}
  ...
  for (const constraint of constraints) {
    if (constraint.criterionId === "site.squareFootage") {
      ...
      if (constraint.min != null && value < constraint.min) {
        failures.push(
          `Square footage ${value} is below required minimum ${constraint.min}.`,
        );
      } else if (constraint.max != null && value > constraint.max) {
        failures.push(
          `Square footage ${value} is above required maximum ${constraint.max}.`,
        );
      } else {
...
function statusFor({ hardFail, unknowns, availability, rankScore, profile }) {
  ...
  if (
    ...
    profile.availability.normalizedStatus === "upcoming"
  ) {
    return "upcoming";
  }
...
function nextQuestionFor(unknowns) {
  ...
  if (reasons.includes("utility")) {
    return {
      ...
      prompt:
        "Confirm the electric distribution utility shown on the site's bill.",
    };
  }
...
function profileCompleteness(user) {
  ...
  return (
    Math.round((values.filter(Boolean).length / values.length) * 100) / 100
  );
}
