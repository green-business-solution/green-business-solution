...
export function calculatePortfolioReadModel({
  ...
  const orderedItemIds = scenarioOrder.length > 0
    ? scenarioOrder
    : Array.isArray(aggregate.itemOrder) && aggregate.itemOrder.length > 0
      ? aggregate.itemOrder
      : Object.keys(aggregate.items || {})
  ...
  const orderedItems = orderedItemIds
    ...
    .map((item) => ({
      ...item,
      independentFinancialValueMinorUnits: Number(item.independentFinancialValueMinorUnits || 0),
      financialSelection: {
        ...item.financialSelection,
        requestedBenefitMinorUnits: Number(item.financialSelection?.requestedBenefitMinorUnits || 0)
      }
    }));
