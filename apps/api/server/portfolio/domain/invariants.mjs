...
export function validateRuleVersion(selection, activeRuleVersion) {
  if ((selection?.calculationBinding || "calc-v1") !== activeRuleVersion) {
    const error = new Error(
      "Expected calculation binding does not match the active scenario binding.",
    );
    error.status = 409;
...
export function validateFinances(selection) {
  const requested = cleanNonNegativeInteger(
    selection?.financialSelection?.requestedBenefitMinorUnits,
  );
  if (requested === null) {
    const error = new Error(
      "financialSelection.requestedBenefitMinorUnits is required.",
    );
    error.status = 400;
  ...
  if (requested > 5_000_000) {
    const error = new Error(
      "financialSelection.requestedBenefitMinorUnits exceeds system limits.",
    );
    error.status = 400;
...
export function validateOwner(user, portfolioId) {
  ...
  if (!isOwner) {
    const error = new Error(
      "Portfolio ownership is invalid for this principal.",
    );
    error.status = 403;
