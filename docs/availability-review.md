...
- `upcoming`: the source indicates a future opening date or upcoming funding round.
...
The matcher treats `active` and `rolling` as pass, `upcoming` as the `upcoming` bucket, `unavailable` as a hard unavailable blocker, and `uncertain` as an unresolved requirement.
...
6. Run `matching:archive-unavailable -- --write-dynamodb --unarchive-restored` so closed opportunities are hidden and reopened opportunities can return.
