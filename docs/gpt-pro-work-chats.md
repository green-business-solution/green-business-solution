...
When local Google OAuth is unavailable, set `RETROFI_GPT_PRO_CHATS_LOCAL_AUTH_BYPASS=1` or use the existing Firstmate tasks local auth bypass so only the GPT Pro work endpoints skip admin sign-in in non-Lambda local runs.
