...
The same contract now also powers the user-preview dashboard sustainability surfaces.
The contract currently exposes water conservation, Scope 1 therm reduction, Scope 2 electricity reduction, Site EUI reduction, grid peak-demand reduction, annual operational CO2e reduction, and annual waste avoided.
Each metric carries per-metric formula IDs, assumptions, quality, trace data, provenance state, and the CO2e-specific Scope 1 and Scope 2 factor ledger plus boundary note so the admin page can show the calculation boundary and fallback sources honestly.
