...
1. Validate these inputs and preserve the source of each supplied value: Existing ice machine type or application; Existing make and model, when available; Existing capacity or size class.
...
4. Return existing certified ice machine performance with its exact unit, or no value when no exact record is supported.
...
* One compatible proposed ice machine result, or no value when the requirements do not identify a supported record
...
2. Download the applicable product export, normalize manufacturer and model details, require one active compatible record, and return only the certified fields for that product family.
3. Reject missing, ambiguous, incompatible, or out-of-scope records instead of inserting a generic default.
4. Return one compatible proposed ice machine result, or no value when the requirements do not identify a supported record.
5. Store the source version, selected record or method, input units, and any warnings with the result.
