...
- **What this category means:** Evaluate operational savings for Thermal energy storage.
- **What the formula calculates:** thermal dispatch -> baseline/proposed bills.
- **What RetroFi asks during ordinary screening:** Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field; Interval timezone and daylight-saving treatment from the uploaded interval artifact; Verified tariff calendar for the modeled interval import energy; the current bill parser has no complete canonical tariff artifact; and 10 more staged screening input(s).
- **What RetroFi resolves automatically:** 4 Bill value(s), 1 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** Thermal-storage schema and pinned dispatch fixture.
- **Why the status is Draft:** Thermal-storage schema and pinned dispatch fixture.
- **Decision requested:** Decide whether RetroFi should keep Thermal-storage interval dispatch at no estimate until this category-specific gate is resolved: Thermal-storage schema and pinned dispatch fixture.
...
- **Limitation:** Thermal-storage schema and pinned dispatch fixture.
...
### Term: `thermal_state_t`

- **Unit:** kWh (`kwh`)
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Thermal state balance with efficiencies and loss applied once
- **Resolved by:** Exact project design and REopt-compatible dispatch method
- **Exact path:** User thermal-storage design; evidence:E-REOPT-DISPATCH
- **Fallback:** None
...
### Term: `standing_loss_t`

- **Unit:** thermal-energy (`thermal_energy`)
- **Dimension:** energy
...
- **Minimum required inputs:** complete interval load and tariff; thermal capacity; charge/discharge performance; standing loss; load coupling.
- **Exact scenario:** insufficient-data.
- **Source fixture:** None.
- **Low/base/high calculation:** Explicit design sensitivities only.
- **Final result path:** thermal dispatch -> baseline/proposed bills
- **Uncertainty:** High.
- **Executable golden fixture:** No.
- **Remaining gate:** Thermal-storage schema and pinned dispatch fixture.
- **Golden result:** None
...
- `chronological_load_and_tariff` - **Required:** Chronological site load, complete tariff, timezone, and analysis-year calendar.
  - **Resolved by:**
    - **User:** Annual thermal-storage bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field
    - **User:** Annual thermal-storage bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Interval timezone and daylight-saving treatment from the uploaded interval artifact
    - **Bill:** Annual thermal-storage bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > utilityExtractedValues rate_schedule and customer_class, verified rather than provider-inferred
...
- Decide whether RetroFi should keep Thermal-storage interval dispatch at no estimate until this category-specific gate is resolved: Thermal-storage schema and pinned dispatch fixture.
