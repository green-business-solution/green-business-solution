...
- **What this category means:** Evaluate operational savings for Fleet charging infrastructure.
- **What the formula calculates:** fleet charging interval load -> unmanaged charging bill.
- **What RetroFi asks during ordinary screening:** Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field; Interval timezone and daylight-saving treatment from the uploaded interval artifact; Verified tariff calendar for the modeled interval import energy; the current bill parser has no complete canonical tariff artifact; and 11 more staged screening input(s).
- **What RetroFi resolves automatically:** 4 Bill value(s), 2 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** Fleet compatibility, EVSE schema, load template, and REopt fixture.
- **Why the status is Draft:** Fleet compatibility, EVSE schema, load template, and REopt fixture.
- **Decision requested:** Decide whether RetroFi should keep Fleet charging added-load bill impact in Draft until this category-specific gate is resolved: Fleet compatibility, EVSE schema, load template, and REopt fixture.
...
- **Limitation:** Fleet compatibility, EVSE schema, load template, and REopt fixture.
...
### Term: `fleet_annual_miles`

- **Unit:** miles/year (`miles_year`)
- **Dimension:** distance_per_time
- **Quantity kind:** activity_rate
- **Formula role:** Annual vehicle activity basis
- **Resolved by:** Exact fleet activity input
- **Exact path:** User fleet annual miles
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Annual fleet miles (User)
...
- **Formula role:** Uses the direct measured intensity or converts combE from kWh/100 miles exactly once
- **Resolved by:** Exact measured intensity or exact FuelEconomy vehicle record
- **Exact path:** User measured kWh per mile; fueleconomy.combE
- **Fallback:** None
...
### Term: `vehicle_kWh_per_mile`

- **Unit:** kWh/mile (`kwh_mile`)
- **Dimension:** energy_per_distance
- **Quantity kind:** intensity
- **Formula role:** Uses the direct measured intensity or converts combE from kWh/100 miles exactly once
- **Resolved by:** Exact measured intensity or exact FuelEconomy vehicle record
...
### Term: `annual_vehicle_kWh`

- **Unit:** kWh/year (`kwh_year`)
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Calculates annual charging energy once
- **Resolved by:** Formula
- **Exact path:** fleet annual miles × vehicle kWh per mile
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Annual fleet miles (User); Vehicle and charger efficiency (Standard)
- **Missing behavior:** NO_ESTIMATE

### Term: `installed_ports`

- **Unit:** count (`count`)
- **Dimension:** count
- **Quantity kind:** count
- **Formula role:** Scopes standby operation without a product-data usage assumption
- **Resolved by:** Exact project port count and schedule
- **Exact path:** User port count and schedule
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Installed port count (User); Vehicle-arrival schedule (User); Vehicle-departure schedule (User)
- **Missing behavior:** OMIT_STANDBY_COMPONENT_OR_NO_ESTIMATE

### Term: `noncharging_hours`
...
### Term: `standby_kW_per_port`

- **Unit:** kW/port (`kw_port`)
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Supplies product standby power only
- **Resolved by:** Exact proposed EVSE rating
...
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Calculates standby energy once
- **Resolved by:** Formula
- **Exact path:** installed ports × standby kW/port × noncharging hours
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** EVSE standby energy
- **Missing behavior:** OMIT_STANDBY_COMPONENT_OR_NO_ESTIMATE
...
- **Formula role:** Calculates unmanaged charging bill impact
...
### Term: `proposed_unmanaged_charging_bill`
...
- **Minimum required inputs:** fleet miles; exact compatible vehicle efficiency; charging schedule; EVSE standby; complete interval load and tariff.
- **Exact scenario:** insufficient-data.
- **Source fixture:** docs/operational-savings-fixtures/sources/fueleconomy-vehicle-schema.json.
- **Low/base/high calculation:** Exact vehicle records and explicit schedules only.
- **Final result path:** fleet charging interval load -> unmanaged charging bill
- **Uncertainty:** High.
- **Executable golden fixture:** No.
- **Remaining gate:** Fleet compatibility, EVSE schema, load template, and REopt fixture.
- **Golden result:** None
...
├─ Rated Charger Power or Capacity (User)
...
- `linked_opportunity` - **Conditional:** Vehicle, class, drive, certification, or minimum-performance restriction when a Linked Opportunity supplies one. Applies only in the documented scenario.
  - **Resolved by:**
    - **Derived:** Annual fleet charging added cost > Linked Opportunity
...
- `energy_star_exact_product` - **Optional:** Exact manufacturer, model, certified rating, and capacity or size when known.
  - **Resolved by:**
    ...
    - **User:** Annual fleet charging added cost > Rated Charger Power or Capacity
...
- `chronological_load_and_tariff` - **Required:** Chronological site load, complete tariff, timezone, and analysis-year calendar.
  - **Resolved by:**
    - **User:** Annual fleet charging added cost > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field
    - **User:** Annual fleet charging added cost > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Interval timezone and daylight-saving treatment from the uploaded interval artifact
    - **Bill:** Annual fleet charging added cost > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > utilityExtractedValues rate_schedule and customer_class, verified rather than provider-inferred
- `reopt_category_constraints` - **Required:** Applicable technology power, energy, efficiency, state, availability, event, or fixed-load-template constraints shown as atomic leaves in the category tree.
  - **Resolved by:**
    ...
    - **User:** Annual fleet charging added cost > Measured kWh per Mile
...
- Decide whether RetroFi should keep Fleet charging added-load bill impact in Draft until this category-specific gate is resolved: Fleet compatibility, EVSE schema, load template, and REopt fixture.
