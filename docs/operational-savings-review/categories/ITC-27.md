...
- **What this category means:** Evaluate operational savings for EV charger installation, Level 2 EV charger installation, DC fast charger installation.
- **What the formula calculates:** added interval load -> proposed bill minus baseline bill.
- **What RetroFi asks during ordinary screening:** Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field; Interval timezone and daylight-saving treatment from the uploaded interval artifact; Verified tariff calendar for the modeled interval import energy; the current bill parser has no complete canonical tariff artifact; and 7 more staged screening input(s).
- **What RetroFi resolves automatically:** 4 Bill value(s), 2 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** EVSE schema fixture, usage profile, and REopt golden fixture.
- **Why the status is Draft:** EVSE schema fixture, usage profile, and REopt golden fixture.
- **Decision requested:** Decide whether RetroFi should keep Public EVSE added-load bill impact at no estimate until this category-specific gate is resolved: EVSE schema fixture, usage profile, and REopt golden fixture.
...
- **Limitation:** EVSE schema fixture, usage profile, and REopt golden fixture.
...
- **Dimension:** count
- **Quantity kind:** count
- **Formula role:** Scopes installed ports
- **Resolved by:** User
...
### Term: `delivered_kWh_t`

- **Unit:** kWh/interval (`kwh_interval`)
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Supplies delivered charging energy and baseline interval load without a product-data usage assumption
- **Resolved by:** Exact project interval inputs
- **Exact path:** User delivered-kWh distribution; Uploaded interval load artifact
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Delivered-kWh distribution per charger (User); Chronological load and tariff
- **Missing behavior:** NO_ESTIMATE

### Term: `baseline_load_t`
...
### Term: `active_efficiency`

- **Unit:** fraction (`fraction`)
- **Dimension:** dimensionless
- **Quantity kind:** ratio
- **Formula role:** Supplies product efficiency and standby power only
...
### Term: `charging_input_kWh_t`

- **Unit:** kWh/interval (`kwh_interval`)
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Builds charging and standby interval load with efficiency applied once
- **Resolved by:** Formula using exact session and interval inputs
- **Exact path:** User session distributions; Formula
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Session-arrival distribution per charger (User); Session-duration distribution per charger (User); EVSE interval load profile
- **Missing behavior:** NO_ESTIMATE

### Term: `noncharging_interval_hours_t`

- **Unit:** hours/interval (`hours_interval`)
- **Dimension:** time
...
- **Formula role:** Calculates added-load bill impact
...
- Decide whether RetroFi should keep Public EVSE added-load bill impact at no estimate until this category-specific gate is resolved: EVSE schema fixture, usage profile, and REopt golden fixture.
