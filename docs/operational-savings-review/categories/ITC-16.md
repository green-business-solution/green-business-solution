...
- **What this category means:** Evaluate operational savings for Automated demand response controls.
- **What the formula calculates:** baseline and proposed interval bills -> USD/year.
- **What RetroFi asks during ordinary screening:** Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field; Interval timezone and daylight-saving treatment from the uploaded interval artifact; Verified tariff calendar for the modeled interval import energy; the current bill parser has no complete canonical tariff artifact; and 6 more staged screening input(s).
- **What RetroFi resolves automatically:** 4 Bill value(s), 1 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** Pinned REopt schema, demand-response counterfactual, and golden fixture.
- **Why the status is Draft:** Pinned REopt schema, demand-response counterfactual, and golden fixture.
- **Decision requested:** Decide whether RetroFi should keep Demand-response interval bill delta at no estimate until this category-specific gate is resolved: Pinned REopt schema, demand-response counterfactual, and golden fixture.
...
- **Limitation:** Pinned REopt schema, demand-response counterfactual, and golden fixture.
...
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Baseline chronological load
- **Resolved by:** Required uploaded interval artifact
...
- **Dimension:** power
- **Quantity kind:** power
- **Formula role:** Constructs proposed interval load
- **Resolved by:** User event constraints and formula
- **Exact path:** User shed and rebound profile
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** Maximum shed kW, if known (User); Rebound or recovery constraint, if known (User)
- **Missing behavior:** NO_ESTIMATE
...
- **Minimum required inputs:** complete interval load; verified tariff; event schedule; shed and rebound constraints.
- **Exact scenario:** insufficient-data.
- **Source fixture:** None.
- **Low/base/high calculation:** Explicit scenarios only.
- **Final result path:** baseline and proposed interval bills -> USD/year
- **Uncertainty:** High.
- **Executable golden fixture:** No.
- **Remaining gate:** Pinned REopt schema, demand-response counterfactual, and golden fixture.
- **Golden result:** None
...
- `chronological_load_and_tariff` - **Required:** Chronological site load, complete tariff, timezone, and analysis-year calendar.
  - **Resolved by:**
    - **User:** Annual bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field
    - **User:** Annual bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Interval timezone and daylight-saving treatment from the uploaded interval artifact
    - **Bill:** Annual bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > utilityExtractedValues rate_schedule and customer_class, verified rather than provider-inferred
...
- Decide whether RetroFi should keep Demand-response interval bill delta at no estimate until this category-specific gate is resolved: Pinned REopt schema, demand-response counterfactual, and golden fixture.
