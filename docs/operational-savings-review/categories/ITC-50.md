...
- **What this category means:** Evaluate operational savings for High-efficiency fryer, High-efficiency oven, High-efficiency steamer.
- **What the formula calculates:** active plus idle resource deltas -> matched rates.
- **What RetroFi asks during ordinary screening:** Count of identical units in project scope; Existing Recognizable Equipment Type or Application; Proposed Product Class or Intended Scope; and 3 more staged screening input(s).
- **What RetroFi resolves automatically:** 3 Bill value(s), 2 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** Fryer, oven, and steamer schema fixtures plus existing baselines and usage units.
- **Why the status is Draft:** Fryer, oven, and steamer schema fixtures plus existing baselines and usage units.
- **Decision requested:** Decide whether RetroFi should keep Commercial cooking tested-duty and idle balance in Draft until this category-specific gate is resolved: Fryer, oven, and steamer schema fixtures plus existing baselines and usage units.
...
- **Limitation:** Fryer, oven, and steamer schema fixtures plus existing baselines and usage units.
...
- **Dimension:** count
- **Quantity kind:** count
- **Formula role:** Annual activity and idle bases without a product-data usage assumption
- **Resolved by:** User exact commercial usage inputs
- **Exact path:** User usage inputs
- **Fallback:** None
- **Evidence:** Direct input or deterministic formula
- **Source location:** In-scope quantity; Recognizable Cooking Usage Pattern; Annual Activity per Unit in the Certified Test Unit (User); Annual Idle Hours per Unit (User)
- **Missing behavior:** NO_ESTIMATE

### Term: `annual_activity_per_unit`

- **Unit:** certified activity-unit/year (`certified_activity_unit_year`)
- **Dimension:** declared
...
### Term: `active_intensity_existing,r`

- **Unit:** resource/certified activity (`resource_certified_activity`)
- **Dimension:** declared
- **Quantity kind:** numeric
- **Formula role:** Existing active and idle intensity
- **Resolved by:** Exact existing rating or measurement
- **Exact path:** User Existing Model
- **Fallback:** None
- **Evidence:** E-ENERGY-STAR-EXISTING-UNSUPPORTED - UNSUPPORTED; E-CCMS-EXISTING-UNSUPPORTED - UNSUPPORTED
- **Source location:** Current qualified-product datasets: No installed-population artifact; No reviewed retained historical export: No installed-population table or historical-retention proof
- **Missing behavior:** NO_ESTIMATE

### Term: `idle_rate_existing,r`

- **Unit:** resource/hour (`resource_hour`)
- **Dimension:** declared
- **Quantity kind:** numeric
- **Formula role:** Existing active and idle intensity
- **Resolved by:** Exact existing rating or measurement
...
### Term: `active_intensity_proposed,r`

- **Unit:** resource/certified activity (`resource_certified_activity`)
- **Dimension:** declared
- **Quantity kind:** numeric
- **Formula role:** Proposed active and idle intensity with efficiency applied once
- **Resolved by:** Exact proposed certified fields and one conversion when needed
...
### Term: `tested_cooking_efficiency`
...
### Term: `useful_test_load_per_unit`

- **Unit:** energy/activity (`energy_activity`)
- **Dimension:** energy
...
### Term: `active_input_per_test_unit`
...
- **Dimension:** energy
- **Quantity kind:** energy
- **Formula role:** Executes to_billed_unit exactly once when the efficiency path is used instead of direct input intensity
...
- **Missing behavior:** NO_ESTIMATE_WHEN_EFFICIENCY_PATH_IS_USED
...
- **Quantity kind:** price
- **Formula role:** Values resource-specific active and idle deltas
- **Resolved by:** Bill
...
- Decide whether RetroFi should keep Commercial cooking tested-duty and idle balance in Draft until this category-specific gate is resolved: Fryer, oven, and steamer schema fixtures plus existing baselines and usage units.
