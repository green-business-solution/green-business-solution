...
**Purpose:**
Use U.S. Environmental Protection Agency - CHP technologies and calculator to resolve annual electricity generation; Annual input fuel; Annual useful recovered heat from the listed category inputs.
...
3. When an exact value is unavailable, use only a source-specific retained population or equation with documented filters, numeric rule, unit, scope, and version; otherwise report the implementation limitation.
4. Return one selected annual electricity generation; Annual input fuel; Annual useful recovered heat.
...
**Purpose:**
Use National Laboratory of the Rockies - REopt V3 and REopt.jl to resolve baseline annual grid and fuel bill; Proposed annual grid and fuel bill from the listed category inputs.
...
* Baseline annual grid and fuel bill
* Proposed annual grid and fuel bill

**How to Use:**

1. Map the Microgrid System inputs to the documented Microgrid System Interval Bill Calculation source fields or model inputs: Timestamped interval utility data from the uploaded utility artifact; Time zone and daylight-saving metadata from the uploaded utility artifact; Included component types; Annual operating profile; Power capacity; Usable-energy capacity; Charge efficiency prescribed by the linked opportunity; Discharge efficiency prescribed by the linked opportunity; Initial state of charge; Reserve constraint; Interval solar generation from the connected PVWatts process; Interval wind generation from the connected wind process; Annual generation, input fuel, and useful recovered heat from the connected onsite-generation process.
...
3. When an exact value is unavailable, use only a source-specific retained population or equation with documented filters, numeric rule, unit, scope, and version; otherwise report the implementation limitation.
4. Return one selected baseline annual grid and fuel bill; Proposed annual grid and fuel bill.
