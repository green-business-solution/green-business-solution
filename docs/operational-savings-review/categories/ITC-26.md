...
- **What this category means:** Evaluate operational savings for Microgrid system.
- **What the formula calculates:** integrated dispatch -> grid and fuel bill difference.
- **What RetroFi asks during ordinary screening:** Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field; Interval timezone and daylight-saving treatment from the uploaded interval artifact; Verified tariff calendar for the modeled interval import energy; the current bill parser has no complete canonical tariff artifact; and 26 more staged screening input(s).
- **What RetroFi resolves automatically:** 1 Profile value(s), 5 Bill value(s), 3 Standard result(s).
- **Conditional Calculation Gates:** None.
- **What remains unsupported:** All component adapters plus integrated golden fixture.
- **Why the status is Draft:** All component adapters plus integrated golden fixture.
- **Decision requested:** Decide whether RetroFi should keep Microgrid composite interval dispatch at no estimate until this category-specific gate is resolved: All component adapters plus integrated golden fixture.
...
- **Limitation:** All component adapters plus integrated golden fixture.
...
### Term: `baseline_grid_and_fuel_bill`

- **Unit:** USD/year (`usd_per_year`)
- **Dimension:** currency_per_time
- **Quantity kind:** currency_flow
- **Formula role:** Final composite bill difference without summing separate category results
- **Resolved by:** Pinned REopt dispatch with PV, wind, CHP, storage, load, tariff, and fuel inputs
- **Exact path:** evidence:E-REOPT-DISPATCH; evidence:E-PVWATTS-V8; evidence:E-WIND-SAM; evidence:E-EPA-CHP
- **Fallback:** None
- **Evidence:** E-REOPT-DISPATCH - UNVERIFIED; E-PVWATTS-V8 - VERIFIED; E-WIND-SAM - UNVERIFIED; E-EPA-CHP - UNVERIFIED
- **Source location:** Versioned REopt input schema, solver, and result schema: Exact version, fields, and golden result are not pinned; Reviewed schema snapshot for GET /api/pvwatts/v8: Required inputs system_capacity, module_type, losses, array_type, tilt, azimuth and location or solar_resource_file; outputs ac, ac_monthly, ac_annual, capacity_factor, station_info.solar_resource_file, warnings; bound fixture fields: system_capacity, module_type, losses, array_type, tilt, azimuth, location_or_solar_resource_file, outputs.ac, outputs.ac_monthly, outputs.ac_annual, outputs.capacity_factor, station_info.solar_resource_file, warnings; SAM wind compute module and Wind Toolkit resource: Exact module variables and resource-field contract are not pinned; Technology and capacity performance tables plus CHP savings calculator workbook: Exact row and workbook cell contract not yet captured
- **Missing behavior:** NO_ESTIMATE

### Term: `proposed_grid_and_fuel_bill`
...
- **Minimum required inputs:** complete load and tariffs; all technology designs; fuel contracts; operating constraints.
- **Exact scenario:** insufficient-data.
- **Source fixture:** None.
- **Low/base/high calculation:** Explicit integrated scenarios only.
- **Final result path:** integrated dispatch -> grid and fuel bill difference
- **Uncertainty:** High.
- **Executable golden fixture:** No.
- **Remaining gate:** All component adapters plus integrated golden fixture.
- **Golden result:** None
...
- Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Avoidable marginal resource price [BR-AVOIDABLE-RESOURCE-RATE] > Liquid or vehicle-fuel price > Documented current project fuel price for the matching fuel and geography
...
- Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Avoidable marginal resource price [BR-AVOIDABLE-RESOURCE-RATE] > Gas volumetric charge > utilityExtractedValues average_cost_per_therm for a verified single volumetric tariff, or gas_delivery_charges and gas_procurement_charges divided by matched therms
...
- `linked_opportunity` - **Conditional:** Product, technology, fuel, capacity, or minimum-performance restriction when a Linked Opportunity supplies one. Applies only in the documented scenario.
  - **Resolved by:**
    - **Derived:** Annual microgrid direct bill reduction > Linked Opportunity
...
- `chronological_load_and_tariff` - **Required:** Chronological site load, complete tariff, timezone, and analysis-year calendar.
  - **Resolved by:**
    - **User:** Annual microgrid direct bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Timestamped Green Button interval kW or kWh artifact; no current canonical bill-dictionary field
    - **User:** Annual microgrid direct bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > Interval timezone and daylight-saving treatment from the uploaded interval artifact
    - **Bill:** Annual microgrid direct bill reduction > Chronological load and tariff [BR-INTERVAL-LOAD-AND-TARIFF] > utilityExtractedValues rate_schedule and customer_class, verified rather than provider-inferred
...
- Decide whether RetroFi should keep Microgrid composite interval dispatch at no estimate until this category-specific gate is resolved: All component adapters plus integrated golden fixture.
