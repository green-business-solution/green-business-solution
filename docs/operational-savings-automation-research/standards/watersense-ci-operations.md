...
| --- | --- | --- | --- | --- | --- |
| meter history | String, identifier, or source enumeration | Not unit-bearing or unit is source-specific | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
| equipment inventory | String, identifier, or source enumeration | Not unit-bearing or unit is source-specific | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
| flow or consumption | Numeric scalar or numeric series | Source-declared volume/time | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
| operating time | String, identifier, or source enumeration | Hours or source-declared time | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
| measured leak flow | Numeric scalar or numeric series | Source-declared volume/time | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
| cooling-tower makeup and blowdown records | Structured record or array | Not unit-bearing or unit is source-specific | Payload, calculation input, or output | Preserve source nulls; reject null only when the process requires the field | Not treated as an enumeration unless the source schema declares one |
...
The source uses the shared registry tables plus these target tables: calculation_assumptions, calculation_runs, selected_value_provenance.

```sql
CREATE TABLE os_watersense_ci_operations_records (
...
CREATE INDEX os_watersense_ci_operations_active_exact_idx
  ON os_watersense_ci_operations_records ((normalized_payload->>'normalized_identifier'), effective_from, effective_to)
  WHERE active;
CREATE INDEX os_watersense_ci_operations_requirements_idx
  ON os_watersense_ci_operations_records USING gin (normalized_payload jsonb_path_ops)
  WHERE active;
```
...
The acquired or inspected source evidence is Inspected Water Use Inventory columns in official workbook.
The retained compact sample is `docs/operational-savings-automation-research/samples/watersense-ci-operations.sample.json`.
The source or model interface inspected is ws-commercial-excel-writeable-tables.xlsx.
The local output kind is `model_result_set`, the selection rule is `PINNED_LOCAL_FORMULA:leakWater`, and the output unit is `gallons/year`.
