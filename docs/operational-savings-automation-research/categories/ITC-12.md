...
- `doe_measur`: In-Scope Equipment Count [User] + Existing Motor Nameplate or Measurement [Project Document] + Proposed motor input or shaft rating [Linked Opportunity] + Annual operating hours from the connected schedule process [Standard Output] -> STD-DOE-MEASUR -> model_versions + model_input_schemas + calculation_assumptions + calculation_runs + calculation_warnings -> Existing motor input power -> existing_input_kW (kW) + Proposed motor input power -> proposed_input_kW (kW)
