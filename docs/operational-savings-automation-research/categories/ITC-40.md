...
- `doe_measur`: In-Scope Equipment Count [User] + Required flow from a nameplate, measurement, audit, or contractor specification [Project Document] + Total dynamic head [Project Document] + Existing Pump Nameplate or Test Information [Project Document] + Existing Motor Nameplate or Test Information [Project Document] + Proposed Pump Specifications [Linked Opportunity] + Proposed Motor Specifications [Linked Opportunity] + Annual operating hours from the connected schedule process [Standard Output] -> STD-DOE-MEASUR -> model_versions + model_input_schemas + calculation_assumptions + calculation_runs + calculation_warnings -> Existing pump input power -> existing_input_kW (kW) + Proposed pump input power -> proposed_input_kW (kW)
